...
storage likewise keep those fixtures persistently escaped.
...
typed callee, never argument or receiver variable names. Creating a slice alias
...
pointer aliases, so later indirect writes invalidate the rooted fixture. Conditional
...
func ps6079FunctionGates(pass *analysis.Pass, function *ast.FuncDecl) []ps6079Gate {
	...
	ast.Inspect(function.Body, func(node ast.Node) bool {
		...
		requirements := ps6079ConditionRequirements(pass, object, statement.Cond, fastWhenTrue)
...
func ps6079ConditionRequirements(pass *analysis.Pass, function *types.Func, expression ast.Expr, want bool) []ps6079Requirement {
	expression = ps2110Unparen(expression)
	switch candidate := expression.(type) {
	case *ast.UnaryExpr:
		if candidate.Op == token.NOT {
			return ps6079ConditionRequirements(pass, function, candidate.X, !want)
		}
	case *ast.BinaryExpr:
		if candidate.Op == token.LAND && want || candidate.Op == token.LOR && !want {
			left := ps6079ConditionRequirements(pass, function, candidate.X, want)
			right := ps6079ConditionRequirements(pass, function, candidate.Y, want)
	...
	case *ast.CallExpr:
		...
		relation, ok := ps6079PredicateRelation(ps6074CalledName(candidate.Fun))
		if !ok {
			return nil
		}
...
func ps6079ReportBenchmark(
	...
	for _, requirement := range route.requires {
		...
		state, event := ps6079FixtureExpression(pass, argument, events, call.Pos())
...
func ps6079FixtureEvents(
	...
	astutil.WithStack(body, func(node ast.Node, stack []ast.Node) bool {
		...
		markAsynchronousReceive := func(destination types.Object, expression ast.Expr) {
			if destination != nil && ps6079ExpressionMayReceiveReference(
		...
		case *ast.AssignStmt:
			...
			if value.Tok != token.ASSIGN && value.Tok != token.DEFINE {
				for _, left := range value.Lhs {
					ps6079UnknownFixtureTarget(pass, left, value.Pos(), "compound assignment", record)
				}
				break
			}
			if len(value.Lhs) != len(value.Rhs) {
				if len(value.Rhs) == 1 {
					component, referenceResults := ps6079MultiResultReferenceComponent(
						pass, value.Rhs[0], len(value.Lhs), referenceAliases,
					)
				...
				for _, left := range value.Lhs {
					ps6079UnknownFixtureTarget(pass, left, value.Pos(), "multi-value assignment", record)
				}
				break
			}
			if len(value.Lhs) == len(value.Rhs) {
				...
				for index, expression := range value.Rhs {
					referenceComponents[index] = ps6079ReferenceAliasComponent(pass, expression, referenceAliases)
				}
				...
				for index, left := range value.Lhs {
					if identifier, ok := ps2110Unparen(left).(*ast.Ident); ok {
						...
						state, event := ps6079AssignedFixtureExpression(pass, value.Rhs[index], events, value.Pos())
					...
					if indexed, ok := ps2110Unparen(left).(*ast.IndexExpr); ok {
						...
						ps6079UnknownFixtureTarget(pass, left, value.Pos(), "indirect fixture write", record)
					}
					ps6079InvalidateAliasSource(pass, value.Rhs[index], value.Pos(), record)
				}
			}
		case *ast.ValueSpec:
			if len(value.Names) != len(value.Values) && len(value.Values) == 1 {
				component, referenceResults := ps6079MultiResultReferenceComponent(
					pass, value.Values[0], len(value.Names), referenceAliases,
				)
			...
			if len(value.Names) == len(value.Values) {
				...
				for index, name := range value.Names {
					...
					state, event := ps6079AssignedFixtureExpression(pass, value.Values[index], events, value.Pos())
		...
		case *ast.CallExpr:
			...
			case *ast.GoStmt:
				for _, object := range ps6079AsynchronousFixtureRoots(pass, value, events) {
					markAsynchronousEscape(object)
					record(object, value.Pos(), ps6079UnknownFixture, "asynchronous fixture escape")
				}
				ps6079MutationEvents(pass, value, events, partialReferenceAliases, true, record)
			default:
				...
				if ps6079CallMayLaunchAsynchronousWork(pass, declarations, value, make(map[*types.Func]bool)) {
					for _, object := range ps6079AsynchronousFixtureRoots(pass, value, events) {
						markAsynchronousEscape(object)
						record(object, value.Pos(), ps6079UnknownFixture, "fixture may escape asynchronously through "+exprTextRendered(value.Fun))
					}
				}
				ps6079MutationEvents(pass, value, events, partialReferenceAliases, false, record)
			}
		case *ast.IncDecStmt:
			ps6079UnknownFixtureTarget(pass, value.X, value.Pos(), "increment or decrement", record)
		case *ast.RangeStmt:
			...
			if functionRange {
				for object := range events {
					...
					if ps6079ContainsReferenceFixture(object.Type()) {
			...
			for index, target := range []ast.Expr{value.Key, value.Value} {
				...
				if ps6079ContainsReferenceFixture(object.Type()) {
					component := ps6079ReferenceAliasComponent(pass, value.X, referenceAliases)
		...
		case *ast.SendStmt:
			for _, object := range ps6079ReferenceAliasSources(pass, value.Value) {
				markAsynchronousEscape(object)
...
func ps6079CallMayLaunchAsynchronousWork(
	...
	callee, _, known := typedCallee(pass, call.Fun)
	if !known {
		return !ps6079BuiltinOrConversion(pass, call.Fun)
	}
...
func ps6079AssignmentMayRetainReference(pass *analysis.Pass, assignment *ast.AssignStmt) bool {
	...
	for _, expression := range assignment.Rhs {
		...
		retainedValue = retainedValue || ps6079ContainsReferenceFixture(typeOf) ||
...
func ps6079AsynchronousFixtureRoots(
	...
	if literal, ok := ps2110Unparen(call.Fun).(*ast.FuncLit); ok {
		...
		ast.Inspect(literal.Body, func(node ast.Node) bool {
			...
			object := pass.TypesInfo.ObjectOf(identifier)
			if object != nil && !locals[object] && len(events[object]) > 0 {
				roots[object] = true
			}
			return true
		})
	} else if callee, _, resolved := typedCallee(pass, call.Fun); resolved && callee.Pkg() != nil &&
...
func ps6079MultiResultReferenceComponent(
	...
	for index := range results {
		referenceResults[index] = ps6079ContainsReferenceFixture(tuple.At(index).Type())
		containsReference = containsReference || referenceResults[index]
	}
	if !containsReference {
		return nil, referenceResults
	}
	return ps6079ReferenceAliasComponent(pass, expression, aliases), referenceResults
}
...
func ps6079AssignedFixtureExpression(
	...
	state, event := ps6079FixtureExpression(pass, expression, events, before)
...
func ps6079MutationEvents(
	...
	name := ps6079FixtureCallName(pass, call.Fun)
	if callee, _, resolved := typedCallee(pass, call.Fun); resolved && callee.Pkg() != nil && pass.Pkg != nil &&
	...
	if typedBuiltinName(pass, call.Fun, "copy") && len(call.Args) >= 2 {
		if object := ps6079RootObject(pass, call.Args[0]); object != nil {
	...
	if typedBuiltinName(pass, call.Fun, "clear") && len(call.Args) == 1 {
		object := ps6079RootObject(pass, call.Args[0])
		switch {
		case object != nil && ps6079SliceType(pass.TypesInfo.TypeOf(call.Args[0])):
		...
		case object != nil && ps6079MapType(pass.TypesInfo.TypeOf(call.Args[0])):
	...
	if typedBuiltinName(pass, call.Fun, "append") && len(call.Args) > 0 {
		...
		for _, object := range ps6079ReferenceAliasSources(pass, call.Args[0]) {
			add(object, call.Pos(), ps6079UnknownFixture, "append may mutate fixture backing storage")
		}
		return
	}
	state, mutates := ps6079MutatorState(name)
	if mutates {
		...
		if destination != nil {
			if object := ps6079RootObject(pass, destination); object != nil {
		...
		if selector, ok := ps2110Unparen(call.Fun).(*ast.SelectorExpr); ok && selector.X != destination {
			for _, object := range ps6079ReferenceAliasSources(pass, selector.X) {
		...
		for _, argument := range call.Args {
			...
			for _, object := range ps6079ReferenceAliasSources(pass, argument) {
	...
	if identifier, ok := ps2110Unparen(call.Fun).(*ast.Ident); ok {
		if _, builtin := pass.TypesInfo.Uses[identifier].(*types.Builtin); builtin {
			return
		}
	}
	if _, _, resolved := typedCallee(pass, call.Fun); !resolved && !ps6079BuiltinOrConversion(pass, call.Fun) {
...
func ps6079FixtureExpression(
	...
	case *ast.UnaryExpr:
		if value.Op == token.SUB {
			state, event := ps6079FixtureExpression(pass, value.X, events, before)
	...
	case *ast.CallExpr:
		...
		if ps6079MathAbsCall(pass, value) {
			if len(value.Args) == 1 {
				state, _ := ps6079FixtureExpression(pass, value.Args[0], events, before)
		...
		name := ps6079FixtureCallName(pass, value.Fun)
		if state, ok := ps6079NamedFixtureState(name); ok {
...
func ps6079FixtureCallName(pass *analysis.Pass, function ast.Expr) string {
	if callee, _, ok := typedCallee(pass, function); ok {
		name := callee.Name()
	...
	return ps6007NormalizeName(ps6074CalledName(function))
}
...
func ps6079ConversionPreservesReference(pass *analysis.Pass, call *ast.CallExpr) bool {
	if len(call.Args) != 1 || !ps6079ReferenceFixture(pass.TypesInfo.TypeOf(call)) ||
		!ps6079ReferenceFixture(pass.TypesInfo.TypeOf(call.Args[0])) {
		return false
	}
	...
	astutil.WithStack(benchmark.Body, func(node ast.Node, stack []ast.Node) bool {
		...
		case *ast.IfStmt:
			...
			if !conditional && value.Pos() > workload.Pos() && missing &&
				...
				ps6079AssertionMustExecute(pass, benchmark, workload.Pos(), value.Pos()) && ps6079BenchmarkFailureBranch(pass, benchmark, value) &&
				ps6079RouteCounterReset(pass, declarations, benchmark, workload, value.Pos(), observation, route) {
				proved = true
				return false
			}
		}
...
func ps6079AssertionMustExecute(pass *analysis.Pass, benchmark *ast.FuncDecl, workload, assertion token.Pos) bool {
	...
	ast.Inspect(benchmark.Body, func(node ast.Node) bool {
		...
		if node == nil || node.Pos() <= workload || node.Pos() >= assertion {
			return true
		}
		switch value := node.(type) {
		case *ast.ReturnStmt:
			mustExecute = false
		case *ast.BranchStmt:
			mustExecute = value.Tok != token.GOTO
		case *ast.CallExpr:
			mustExecute = !ps6079PanicCall(pass, value)
		}
		return mustExecute
	})
	return mustExecute
...
func ps6079BenchmarkBranchMayBypassFailure(pass *analysis.Pass, body *ast.BlockStmt, receiver types.Object) bool {
	...
	ast.Inspect(body, func(node ast.Node) bool {
		...
		if ok && (selector.Sel.Name == "Skip" || selector.Sel.Name == "Skipf" || selector.Sel.Name == "SkipNow") {
			bypasses = ps6079RootObject(pass, selector.X) == receiver
			if !bypasses {
				selection := pass.TypesInfo.Selections[selector]
				bypasses = selection != nil && selection.Kind() == types.MethodExpr && len(call.Args) > 0 &&
		...
		if ps6079FailureCall(pass, call, receiver) {
			// Inspect argument expressions because they run before the failure call.
			return true
		}
		if ps6079PanicCall(pass, call) || !ps6079BuiltinOrConversion(pass, call.Fun) {
			// Any opaque call may terminate, skip, or exit before a later lexical
			// failure marker. Only builtins/conversions and the actual benchmark
			// failure calls are trusted to preserve the proof.
			bypasses = true
			return false
		}
		return true
...
func ps6079FailureCall(pass *analysis.Pass, call *ast.CallExpr, receiver types.Object) bool {
	...
	return ps6079RootObject(pass, selector.X) == receiver
}
