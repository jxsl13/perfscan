...
// TestEquiv_PS2047PanicContract pins both sides of the auto-fix boundary:
// integer formatters share a panic value for invalid bases. Go 1.26 also used
// a shared float panic, while Go 1.27 deliberately names FormatFloat and
// AppendFloat differently. PS2047 and PS2136 therefore fix FormatFloat only
// for constant 32/64 bitSize across the supported toolchain matrix.
func TestEquiv_PS2047PanicContract(t *testing.T) {
	...
	for _, bs := range []int{0, 16, 33, 128} {
		...
		if b == nil || a == nil {
			t.Fatalf("FormatFloat/AppendFloat(bitSize %d): expected non-nil panic values, got %v vs %v", bs, b, a)
		}
		shared := b == "strconv: illegal AppendFloat/FormatFloat bitSize" && a == b
		split := b == "strconv: illegal FormatFloat bitSize" && a == "strconv: illegal AppendFloat bitSize"
		if !shared && !split {
			t.Fatalf("FormatFloat/AppendFloat(bitSize %d): unknown supported-toolchain panic contract: %v vs %v", bs, b, a)
		}
	}
}
