...
// TestEquivPS5047 proves the rewrite is byte-identical: fmt.Appendf(dst,
// "%b", n) and strconv.AppendInt/AppendUint(dst, ...) append the same base-2 (binary)
// digits for every integer value and width.
func TestEquivPS5047(t *testing.T) {
	ci := func(v int64) {
		a := fmt.Appendf([]byte("seed"), "%b", v)
		b := strconv.AppendInt([]byte("seed"), v, 2)
		if string(a) != string(b) {
			t.Fatalf("int64 %b: fmt=%s strconv=%s", v, a, b)
		}
	}
	cu := func(v uint64) {
		a := fmt.Appendf([]byte("seed"), "%b", v)
		b := strconv.AppendUint([]byte("seed"), v, 2)
		if string(a) != string(b) {
			t.Fatalf("uint64 %b: fmt=%s strconv=%s", v, a, b)
		}
	}
