...
func formatFloatDynamic(f float64, bitSize int) []byte {
	return []byte(strconv.FormatFloat(f, 'g', -1, bitSize)) // want `\[\]byte\(strconv\.FormatFloat\(\.\.\.\)\) allocates a string then copies it; strconv\.AppendFloat\(nil, \.\.\.\) writes the bytes directly; bitSize is not a compile-time 32 or 64`
}

func formatFloatInvalid(f float64) []byte {
	return []byte(strconv.FormatFloat(f, 'g', -1, 16)) // want `\[\]byte\(strconv\.FormatFloat\(\.\.\.\)\) allocates a string then copies it; strconv\.AppendFloat\(nil, \.\.\.\) writes the bytes directly; bitSize is not a compile-time 32 or 64`
}
