...
func exact(dst, src []float32) {
	for i := range dst { // want `exact float32 Abs loop src\[i\] -> float64 -> math.Abs -> float32 -> dst\[i\] remains scalar; native SIMD must match a same-toolchain scalar raw-bit golden`
