...
func nativeProducer(path string) string {
	return filepath.FromSlash(filepath.ToSlash(filepath.FromSlash(filepath.Clean(path)))) // want `filepath.FromSlash restores the already-native filepath producer after 2 mixed slash-normalizer layer`
}
