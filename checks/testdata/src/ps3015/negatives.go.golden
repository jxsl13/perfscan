...
// slices.SortedFunc; none is reported, and none is rewritten.
func negatives(ys []string, xs []int, base string) {
	// Swapped operands: strings.Compare(b, a) is a DESCENDING sort — never
	// slices.Sorted.
	fmt.Println(slices.SortedFunc(slices.Values(ys), func(a, b string) int { return strings.Compare(b, a) }))

	// Extra work in the body: not a bare comparator.
	fmt.Println(slices.SortedFunc(slices.Values(ys), func(a, b string) int {
		fmt.Println(a, b)
		return strings.Compare(a, b)
	}))

	// A captured variable instead of the second parameter.
	fmt.Println(slices.SortedFunc(slices.Values(ys), func(a, b string) int { return strings.Compare(a, base) }))
	...
	ns := []name{"b", "a"}
	fmt.Println(slices.SortedFunc(slices.Values(ns), func(a, b name) int { return strings.Compare(string(a), string(b)) }))

	// cmp.Compare is a different package's comparator; PS3012 owns that
	// spelling.
	...
	fmt.Println(slices.SortedFunc(slices.Values(ys), cmpFn))

	// slices.SortFunc is the eager sibling — PS3009 owns that callee.
...
func shadowedStrings(ys []string) []string {
	strings := fakeStrings{}
	return slices.SortedFunc(slices.Values(ys), func(a, b string) int { return strings.Compare(a, b) })
}
