...
// Inside the receiver's own Write method the rewrite fmt.Fprintf(h, ...)
// would write through h.Write — the enclosing method itself: unbounded
// recursion that still compiles. Nothing is reported.
type selfH struct{ b []byte }

func (h *selfH) WriteString(s string) (int, error) {
	h.b = append(h.b, s...)
	return len(s), nil
}

func (h *selfH) Write(p []byte) (int, error) {
	return h.WriteString(fmt.Sprintf("n=%d", len(p)))
}
...
func (h *selfH) dump(v int) {
	h.WriteString(fmt.Sprintf("%d", v)) // want `WriteString\(fmt\.Sprintf\(\.\.\.\)\) builds a throwaway string; fmt\.Fprintf\(w, \.\.\.\) writes the formatted bytes directly to w`
}

// A DIFFERENT writer inside Write is still reported: sb is not the
// receiver.
func (h *selfH) mirror(sb *strings.Builder, p []byte) (int, error) {
	return sb.WriteString(fmt.Sprintf("%x", p)) // want `WriteString\(fmt\.Sprintf\(\.\.\.\)\) builds a throwaway string; fmt\.Fprintf\(w, \.\.\.\) writes the formatted bytes directly to w`
}
