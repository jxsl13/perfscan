...
func addImport(buf *bytes.Buffer, a, b string) {
	fmt.Fprintln(buf, a, b) // want `fmt\.Fprintln over only plain strings writes exactly one space between operands and one trailing newline, boxing every operand through fmt's machinery; io\.WriteString\(w, a\+" "\+b\+"\\n"\) writes the identical bytes with the same \(n, err\)`
	fmt.Fprintf(buf, "%d\n", buf.Len())
}
