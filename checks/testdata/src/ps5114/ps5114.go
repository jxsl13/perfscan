...
func join(root, tail string) string {
	return filepath.FromSlash(filepath.Join(root, tail))
}

func emptyJoin() string {
	return filepath.FromSlash(filepath.Join())
}
...
func deep(name string) string {
	return filepath.FromSlash(filepath.FromSlash(filepath.FromSlash(filepath.Base(name)))) // want `filepath.FromSlash rescans the already-native result of filepath.Base through 3 redundant FromSlash layer`
}
