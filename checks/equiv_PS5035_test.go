...
// Runtime differential for PS5035: strings.TrimFunc(s, unicode.IsSpace)
...
// ps5035Before is the exact Before-shape of the check; ps5035After is
// the exact After-shape the fix emits. They must agree byte-for-byte on
// EVERY input.
func ps5035Before(s string) string { return strings.TrimFunc(s, unicode.IsSpace) }
func ps5035After(s string) string  { return strings.TrimSpace(s) }

func ps5035Check(t *testing.T, s string) {
	t.Helper()
	if before, after := ps5035Before(s), ps5035After(s); before != after {
		t.Fatalf("TrimSpace divergence on %q:\n before=%q\n after=%q", s, before, after)
	}
}

func TestEquiv_PS5035_ExhaustiveShort(t *testing.T) {
	alphabet := []byte{
...
func TestEquiv_PS5035_TargetedSeeds(t *testing.T) {
...
func TestEquiv_PS5035_Randomized(t *testing.T) {
...
// TestEquiv_PS5035_NoAlloc pins the perf premise: neither spelling
// allocates — both return a substring of s.
func TestEquiv_PS5035_NoAlloc(t *testing.T) {
	s := "  \t padded payload with interior  spaces \u00a0 "
	var out string
	if avg := testing.AllocsPerRun(100, func() { out = ps5035Before(s) }); avg != 0 {
		t.Errorf("TrimFunc allocates: %v allocs/op", avg)
	}
	if avg := testing.AllocsPerRun(100, func() { out = ps5035After(s) }); avg != 0 {
