...
The result's CAPACITY differs — slices.Concat sizes to exactly the total
length (cap == len) while the chained append over-allocates by geometric
growth — and that difference is observable if the result is later appended
to and the original is then mutated (the over-allocated chain may reuse its
backing array; the exact-sized Concat reallocates). This is deliberately
WITHIN perfscan's bit-identical bar: it is the identical class of change the
flagship pre-sizing checks (PS2101/PS2104) make — make([]T, 0, n) likewise
clamps cap to the exact size versus an unsized slice's geometric growth. The
capacity of a freshly built slice is an unspecified Go implementation detail
(append's growth is not guaranteed by the spec), so no correct program relies
on it; a rewrite that changes only it is behavior-preserving. (The observable
things that DO matter — contents, nil-ness, and aliasing WITH the operands —
are all preserved, as above; the differential TestEquiv_Concat pins them and
documents the accepted cap divergence.)
