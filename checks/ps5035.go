...
// PS5035 reports strings.TrimFunc(s, unicode.IsSpace) — the generic
...
var PS5035 = register(&lint.Check{
	ID:       "PS5035",
	Category: "indirect",
...
to strings.TrimSpace(s) on the next run with no oscillation.`,
		...
		MeasuredWin: `BenchmarkPS5035 (Apple M2 Pro, go1.26): a 51-byte
...
const ps5035Msg = "strings.TrimFunc(s, unicode.IsSpace) decodes every boundary rune and calls unicode.IsSpace through a function pointer; strings.TrimSpace(s) is the identical trim (it delegates to this very TrimFunc call past its fast path) with an ASCII byte-table fast path"

func runPS5035(pass *analysis.Pass) (any, error) {
	for _, f := range pass.Files {
		...
		ast.Inspect(f, func(n ast.Node) bool {
			...
			sel, matched := ps5035Match(pass, call)
			if !matched {
				return true
			}
			fix := ps5035Fix(f, call, sel)
			if fix != nil {
				fixable++
			}
...
// ps5035Match matches call against strings.TrimFunc(s, unicode.IsSpace):
...
func ps5035Match(pass *analysis.Pass, call *ast.CallExpr) (sel *ast.SelectorExpr, ok bool) {
...
// ps5035Fix builds the strings.TrimSpace(s) rewrite for one site, or nil
...
// file.
func ps5035Fix(f *ast.File, call *ast.CallExpr, sel *ast.SelectorExpr) *analysis.SuggestedFix {
	src := call.Args[0]
