...
// PS5042 reports fmt.Appendf(buf, "%d", n) — a lone "%d" verb formatting one
// integer into a byte buffer — where strconv.AppendInt / strconv.AppendUint
// appends the identical decimal digits without fmt's format parse or interface
// boxing. The "%d" sibling of PS2035 (which owns the "%v" scalar form, whose
// integer arm produces these same digits) and PS2141/PS5040/PS5041.
var PS5042 = register(&lint.Check{
	ID:       "PS5042",
	Category: "alloc",
	Slug:     "appendf-d-int",
	...
	Doc: lint.Documentation{
		Title: `fmt.Appendf(buf, "%d", n) runs fmt's formatter to print one integer; strconv.AppendInt/AppendUint writes the decimal digits directly`,
		Text: `fmt.Appendf(buf, "%d", n) parses the format string, boxes n into an
...
without a fix.`,
		Before: `buf = fmt.Appendf(buf, "%d", n)`,
		After:  `buf = strconv.AppendInt(buf, int64(n), 10)`,
		MeasuredWin: `fmt.Appendf(buf, "%d", n) ~31.5 ns/op vs strconv.AppendInt(buf, int64(n), 10) ` +
			`~8.9 ns/op (~3.6x, Apple M2 Pro, go1.26) — the win is the eliminated format parse ` +
	...
	Analyzer: &analysis.Analyzer{
		Name: "PS5042",
		Doc:  "fmt.Appendf(buf, \"%d\", n) of an integer instead of strconv.AppendInt/AppendUint(buf, ...)",
		Run:  runPS5042,
	},
})

func runPS5042(pass *analysis.Pass) (any, error) {
	for _, f := range pass.Files {
		...
		astutil.WithStack(f, func(n ast.Node, stack []ast.Node) bool {
			...
			if v, err := strconv.Unquote(lit.Value); err != nil || v != "%d" {
				return true
			}
			appendName, prefix, suffix, ok := ps5042IntCase(pass, call.Args[2])
			if !ok {
				return true
			}
			...
			if ps2141ByteSliceUnnamed(pass.TypesInfo.TypeOf(call.Args[0])) &&
				...
				if usable && !(needImport && ps2107ImportsC(f)) {
					...
					fix = &analysis.SuggestedFix{
						Message:   "replace fmt.Appendf(buf, \"%d\", n) with " + useName + "." + appendName,
						TextEdits: edits,
					}
					fixable++
				}
		...
		for _, st := range sites {
			diag := analysis.Diagnostic{
				Pos:     st.call.Pos(),
				End:     st.call.End(),
				Message: "fmt.Appendf(buf, \"%d\", n) parses the format and boxes n to print one integer; strconv.AppendInt/AppendUint appends the decimal digits directly",
			}
...
// ps5042IntCase classifies an unnamed predeclared integer operand into the
// strconv.Append{Int,Uint} call scaffolding: the callee name and the prefix
// (replacing `, "%d", `) and suffix (replacing the closing `)`) spliced around
...
// skipped (its Format method could hijack "%d").
func ps5042IntCase(pass *analysis.Pass, arg ast.Expr) (appendName, prefix, suffix string, ok bool) {
