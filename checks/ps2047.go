...
func runPS2047(pass *analysis.Pass) (any, error) {
	for _, f := range pass.Files {
		ast.Inspect(f, func(n ast.Node) bool {
			...
			} else if !ps2136SafeFloatBitSize(pass, name, inner.Args) {
				diag.Message = msg + "; bitSize is not a compile-time 32 or 64, and FormatFloat/AppendFloat have different panic values for invalid bitSize — kept advisory to preserve recover-observable behavior"
