...
func missingVocab(c *lint.Check, cfg *config.Config) []string {
	...
	fields := map[string]int{
		...
		"outputWorkspaceContracts":          config.UsableOutputWorkspaceContractCount(cfg.OutputWorkspaceContracts),
		"unusedProjectionScratchContracts":  config.UsableUnusedProjectionScratchContractCount(cfg.UnusedProjectionScratchContracts),
...
func validStateExpandedLookupContracts(contracts []config.StateExpandedLookupContract) int {
	return config.UsableStateExpandedLookupContractCount(contracts)
}
...
func validCrossStepAcceleratorResidencyContracts(contracts []config.CrossStepAcceleratorResidencyContract) int {
	valid := 0
	for index := range contracts {
		if contracts[index].Valid() {
			valid++
...
func validRowLocalSparseGatherContracts(contracts []config.RowLocalSparseGatherContract) int {
	valid := 0
	for index := range contracts {
		if contracts[index].Valid() {
			valid++
