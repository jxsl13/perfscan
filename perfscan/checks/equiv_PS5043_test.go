...
// TestEquivPS5043 proves the rewrite is byte-identical: fmt.Appendf(dst,
// "%x", n) and strconv.AppendInt/AppendUint(dst, ...) append the same base-16 (lowercase hex)
// digits for every integer value and width.
func TestEquivPS5043(t *testing.T) {
	ci := func(v int64) {
		a := fmt.Appendf([]byte("seed"), "%x", v)
		b := strconv.AppendInt([]byte("seed"), v, 16)
		if string(a) != string(b) {
			t.Fatalf("int64 %x: fmt=%s strconv=%s", v, a, b)
		}
	}
	cu := func(v uint64) {
		a := fmt.Appendf([]byte("seed"), "%x", v)
		b := strconv.AppendUint([]byte("seed"), v, 16)
		if string(a) != string(b) {
			t.Fatalf("uint64 %x: fmt=%s strconv=%s", v, a, b)
		}
	}
