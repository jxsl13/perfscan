...
// PS5026 reports bytes.ContainsRune(b, 'x') where the rune is a
// compile-time constant in the ASCII range [0, 0x80): ContainsRune is
// IndexRune(b, r) >= 0, and for an ASCII r IndexRune immediately
// delegates to IndexByte(b, byte(r)) — two non-inlined wrapper frames
// before the byte scan. bytes.IndexByte(b, 'x') >= 0 answers the
// identical membership question directly. The BYTES twin of PS5024
// (strings.ContainsRune of a constant ASCII rune).
var PS5026 = register(&lint.Check{
	ID:       "PS5026",
	Category: "arith",
	Slug:     "bytes-containsrune-const-ascii-rune",
	Level:    lint.LevelIdiomatic,
	AutoFix:  true,
	Doc: lint.Documentation{
		Title: "bytes.ContainsRune of a constant ASCII rune chains two wrapper frames before the byte scan; IndexByte >= 0 answers the same membership question directly",
		Text: `bytes.ContainsRune(b, r) is defined as
...
(bytes stays imported).`,
		Before: `if bytes.ContainsRune(b, '/') { ... }
found := !bytes.ContainsRune(line, '=')`,
		After: `if bytes.IndexByte(b, '/') >= 0 { ... }
found := bytes.IndexByte(line, '=') < 0`,
		MeasuredWin: `BenchmarkPS5026 (61-byte haystack, needle in the last
...
the compiler folds it to a byte constant.`,
	...
	Analyzer: &analysis.Analyzer{
		Name: "PS5026",
		Doc:  "bytes.ContainsRune of a constant ASCII rune instead of the direct bytes.IndexByte comparison",
		Run:  runPS5026,
	},
})

func runPS5026(pass *analysis.Pass) (any, error) {
	for _, f := range pass.Files {
		astutil.WithStack(f, func(n ast.Node, stack []ast.Node) bool {
			...
			m := ps5026Match(pass, call)
			if m == nil {
				return true
			}

			// Context bookkeeping for the introduced comparison operator,
			// exactly as in PS5024 (the strings twin) and PS5016/PS5022.
			...
			repl := "bytes.IndexByte(b, " + m.argText + ") " + op
			if needParens {
				repl = "(" + repl + ")"
			}
			msg := "bytes.ContainsRune of the constant ASCII rune " + m.argText +
			...
			case m.lit == nil:
				...
				diag.Message = msg + "; the needle is a constant expression, not a rune literal — rewrite to bytes.IndexByte with a byte conversion by hand"
			case goDefer:
...
// ps5026Site is a matched bytes.ContainsRune call with a constant ASCII
...
type ps5026Site struct {
	sel     *ast.SelectorExpr
	lit     *ast.BasicLit
	argText string
}

// ps5026Match matches a call to the standard library's package-level
// bytes.ContainsRune whose second argument is a compile-time constant
// rune with value in [0, 0x80). Type information pins the callee (a
// shadowed bytes identifier, a same-named method, or a third-party
...
// strings.ContainsRune is PS5024's pattern and bytes.IndexRune is
// PS5023's; both are out of scope here.
func ps5026Match(pass *analysis.Pass, call *ast.CallExpr) *ps5026Site {
	...
	if !ok || fn.Pkg() == nil || fn.Pkg().Path() != "bytes" || fn.Name() != "ContainsRune" {
		return nil
	}
	...
	return &ps5026Site{sel: sel, lit: lit, argText: argText}
}
