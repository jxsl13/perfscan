...
// equal-but-distinguishable ties an unstable sort may arrange either way, so
// byte-identity is not contractual — advisory, no fix.
func sortedFloats(fs []float64) []float64 {
	return slices.SortedFunc(slices.Values(fs), cmp.Compare) // want `slices\.Sorted collects and sorts the float64 elements with the identical ascending order and the comparison inlined \(no auto-fix: float ties -0\.0/\+0\.0 and NaN payloads are equal-but-distinguishable under an unstable sort\)`
}
...
func sortedAny[T cmp.Ordered](xs []T) []T {
	return slices.SortedFunc(slices.Values(xs), func(a, b T) int { return cmp.Compare(a, b) }) // want `slices\.Sorted collects and sorts the T elements with the identical ascending order and the comparison inlined \(no auto-fix: type-parameter element, instantiations may include floats whose ties are equal-but-distinguishable\)`
}
...
func sortedCommented(xs []int) []int {
	return slices.SortedFunc(slices.Values(xs), /* keep me */ cmp.Compare) // want `slices\.Sorted collects and sorts the int elements`
}
...
func negatives(xs []int, ys []string, base int) {
	...
	// slices.Sorted.
	fmt.Println(slices.SortedFunc(slices.Values(xs), func(a, b int) int { return cmp.Compare(b, a) }))

	// Extra work in the body: not a bare comparator.
	fmt.Println(slices.SortedFunc(slices.Values(xs), func(a, b int) int {
		fmt.Println(a, b)
		return cmp.Compare(a, b)
	}))

	// A captured variable instead of the second parameter.
	fmt.Println(slices.SortedFunc(slices.Values(xs), func(a, b int) int { return cmp.Compare(a, base) }))
	...
	fmt.Println(slices.SortedFunc(slices.Values(xs), cmpFn))
...
func shadowedCmp(xs []int) []int {
	cmp := fakeCmp{}
	return slices.SortedFunc(slices.Values(xs), func(a, b int) int { return cmp.Compare(a, b) })
}
