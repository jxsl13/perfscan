...
func (fakeSlices) SortFunc(s []int, f func(a, b int) int) {}
...
func negatives(xs []int, ys []item, base int) {
	...
	slices.SortFunc(xs, func(a, b int) int { if a < b { return -1 }; if a > b { return 1 }; return 1 })

	// SAME direction twice (b > a is a<b again): a>b falls through to 0 —
	// not a three-way, and not slices.Sort's order.
	slices.SortFunc(xs, func(a, b int) int { if a < b { return -1 }; if b > a { return 1 }; return 0 })
	...
	slices.SortFunc(xs, func(a, b int) int { if a < b { return math.MinInt }; if a > b { return 1 }; return 0 })

	// A local named constant is equally opaque to the literal-only shape.
	slices.SortFunc(xs, func(a, b int) int { if a < b { return negOne }; if a > b { return 1 }; return 0 })

	// A captured variable instead of the second parameter.
	slices.SortFunc(xs, func(a, b int) int { if a < base { return -1 }; if a > base { return 1 }; return 0 })

	// Field selectors: the comparison is not on the bare parameters (this
	// is PS2110/keyed-sort territory, and struct ties are observable).
	slices.SortFunc(ys, func(a, b item) int { if a.id < b.id { return -1 }; if a.id > b.id { return 1 }; return 0 })

	// Extra work in the body: not the bare three-way.
	slices.SortFunc(xs, func(a, b int) int { fmt.Println(a, b); if a < b { return -1 }; if a > b { return 1 }; return 0 })

	// A repeated parameter (a < a) is not a comparison of the two.
	slices.SortFunc(xs, func(a, b int) int { if a < a { return -1 }; if a > b { return 1 }; return 0 })

	// A tagged switch is not the expressionless three-way switch.
	slices.SortFunc(xs, func(a, b int) int { switch true { case a < b: return -1; case a > b: return 1 }; return 0 })

	// A named comparator value is opaque — only a fresh literal matches.
	slices.SortFunc(xs, namedCmp)
...
// A shadowed slices resolves SortFunc to a METHOD of the local object, not
