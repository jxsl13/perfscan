...
func sprintUint(u uint) string {
	return fmt.Sprintf("%v", u) // want `fmt\.Sprint\(x\) / fmt\.Sprintf\("%v", x\) on a bool/int/float pays fmt's reflection for a value strconv formats directly`
}

// A plain bool: fixed to strconv.FormatBool.
func sprintBool(b bool) string {
	return fmt.Sprint(b) // want `fmt\.Sprint\(x\) / fmt\.Sprintf\("%v", x\) on a bool/int/float pays fmt's reflection for a value strconv formats directly`
}

// A plain float64 under "%v": fixed to the shortest-'g' FormatFloat form.
func sprintfVFloat64(f float64) string {
	return fmt.Sprintf("%v", f) // want `fmt\.Sprint\(x\) / fmt\.Sprintf\("%v", x\) on a bool/int/float pays fmt's reflection for a value strconv formats directly`
}
...
func sprintFloat32(f32 float32) string {
	return fmt.Sprint(f32) // want `fmt\.Sprint\(x\) / fmt\.Sprintf\("%v", x\) on a bool/int/float pays fmt's reflection for a value strconv formats directly`
}

// A plain float64 under Sprint: fixed like the "%v" spelling.
func sprintFloat(f float64) string {
	return fmt.Sprint(f) // want `fmt\.Sprint\(x\) / fmt\.Sprintf\("%v", x\) on a bool/int/float pays fmt's reflection for a value strconv formats directly`
}
