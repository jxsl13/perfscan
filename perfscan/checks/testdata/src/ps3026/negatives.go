...
func (fakeSlices) SortedFunc(seq func(func(int) bool), f func(a, b int) int) []int { return nil }
...
func negatives(xs []int, ys []item, base int) {
	...
	fmt.Println(slices.SortedFunc(slices.Values(xs), func(a, b int) int { return a - b }))
	...
	fmt.Println(slices.SortedFunc(slices.Values(xs), func(a, b int) int { if a < b { return -1 }; if a > b { return 1 }; return 1 }))

	// SAME direction twice (b > a is a<b again): a>b falls through to 0 —
	// not a three-way, and not slices.Sorted's order.
	fmt.Println(slices.SortedFunc(slices.Values(xs), func(a, b int) int { if a < b { return -1 }; if b > a { return 1 }; return 0 }))
	...
	fmt.Println(slices.SortedFunc(slices.Values(xs), func(a, b int) int { if a < b { return math.MinInt }; if a > b { return 1 }; return 0 }))

	// A local named constant is equally opaque to the literal-only shape.
	fmt.Println(slices.SortedFunc(slices.Values(xs), func(a, b int) int { if a < b { return negOne }; if a > b { return 1 }; return 0 }))

	// A captured variable instead of the second parameter.
	fmt.Println(slices.SortedFunc(slices.Values(xs), func(a, b int) int { if a < base { return -1 }; if a > base { return 1 }; return 0 }))

	// Field selectors: the comparison is not on the bare parameters (a
	// keyed sort whose struct ties are observable).
	fmt.Println(slices.SortedFunc(slices.Values(ys), func(a, b item) int { if a.id < b.id { return -1 }; if a.id > b.id { return 1 }; return 0 }))

	// Extra work in the body: not the bare three-way.
	fmt.Println(slices.SortedFunc(slices.Values(xs), func(a, b int) int { fmt.Println(a, b); if a < b { return -1 }; if a > b { return 1 }; return 0 }))

	// A repeated parameter (a < a) is not a comparison of the two.
	fmt.Println(slices.SortedFunc(slices.Values(xs), func(a, b int) int { if a < a { return -1 }; if a > b { return 1 }; return 0 }))

	// A tagged switch is not the expressionless three-way switch.
	fmt.Println(slices.SortedFunc(slices.Values(xs), func(a, b int) int { switch true { case a < b: return -1; case a > b: return 1 }; return 0 }))

	// A named comparator value is opaque — only a fresh literal matches.
	fmt.Println(slices.SortedFunc(slices.Values(xs), namedCmp))

	// SortedStableFunc is a different callee (the stable collecting sort);
	// unmatched.
...
// A shadowed slices resolves SortedFunc to a METHOD of the local object, not
// the stdlib package function — never matched.
func shadowedSlices(seq func(func(int) bool)) []int {
	slices := fakeSlices{}
	return slices.SortedFunc(seq, func(a, b int) int { if a < b { return -1 }; if a > b { return 1 }; return 0 })
}
