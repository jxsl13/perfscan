...
func sortAny[T cmp.Ordered](s []T) {
	slices.SortStableFunc(s, func(a, b T) int { if a < b { return -1 }; if a > b { return 1 }; return 0 }) // want `slices\.Sort sorts the T elements ascending with a single inlined comparison and no stability cost \(no auto-fix: type-parameter element, instantiations may include floats — NaN makes this comparator inconsistent and slices\.Sort orders NaNs first\)`
	fmt.Println(s)
}
...
func sortCommented(xs []int) {
	slices.SortStableFunc(xs, func(a, b int) int { // want `slices\.Sort sorts the int elements with the identical ascending order, a single inlined comparison and no stability cost`
		if a < b {
			return -1
		}
		if a > b {
