...
// A struct-field source is a selector, not a plain local ident: its
// nil-ness is observable (a field can be nil or non-nil empty), so the
// []T(nil) form's nil/empty divergence from bytes.Clone cannot be ruled
// out — reported, no fix. Mirrors the fasthttp corpus shape
// append([]byte(nil), u.scheme...).
func advisoryFieldNil(c *cloneBuf) []byte {
	return append([]byte(nil), c.data...) // want `append\(\[\]byte\(nil\), c\.data\.\.\.\) is a hand-rolled clone; bytes\.Clone\(c\.data\) says it directly`
}

// The []T{} form over a struct field is equally advisory: a nil field
// diverges from slices.Clone (non-nil empty vs nil). Mirrors the fasthttp
// shape append([]*clientConn{}, c.conns...).
func advisoryFieldEmpty(c *cloneBuf) []int {
	return append([]int{}, c.items...) // want `append\(\[\]int\{\}, c\.items\.\.\.\) is a hand-rolled clone; slices\.Clone\(c\.items\) says it directly`
}
...
// make with a CONSTANT-ZERO length is non-nil but EMPTY: append([]int(nil), s...)
// yields nil where slices.Clone(s) stays non-nil, so this stays advisory even
// though the length is a constant. Pins the "POSITIVE constant length" boundary
// of the guard (a loosening to just "constant length" would reintroduce the
// nil-ness divergence).
func advisoryConstZeroMake() []int {
	s := make([]int, 0)
	return append([]int(nil), s...) // want `append\(\[\]int\(nil\), s\.\.\.\) is a hand-rolled clone; slices\.Clone\(s\) says it directly`
}

// An EMPTY composite literal is non-nil but empty: same []T(nil) divergence,
// advisory. Pins the "literal WITH ELEMENTS" boundary.
func advisoryEmptyLiteral() []int {
	s := []int{}
	return append([]int(nil), s...) // want `append\(\[\]int\(nil\), s\.\.\.\) is a hand-rolled clone; slices\.Clone\(s\) says it directly`
}
