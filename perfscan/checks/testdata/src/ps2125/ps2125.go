...
func runeCountExpr(s string) int {
	n := len([]rune(s)) + 1 // want `len\(\[\]rune\(s\)\) decodes and allocates the whole rune slice just to count it; utf8\.RuneCountInString\(s\) is the bit-identical count with zero allocation`
	return n
}

// Call-argument context.
func runeArg(s string) {
	process(len([]rune(s))) // want `len\(\[\]rune\(s\)\) decodes and allocates the whole rune slice just to count it; utf8\.RuneCountInString\(s\) is the bit-identical count with zero allocation`
}

// An untyped string constant operand defaults to string.
func runeLit() int {
	return len([]rune("héllo wörld")) // want `len\(\[\]rune\("héllo wörld"\)\) decodes and allocates the whole rune slice just to count it; utf8\.RuneCountInString\("héllo wörld"\) is the bit-identical count with zero allocation`
}

// []int32 spells the identical []rune type.
func runeInt32(s string) int {
	return len([]int32(s)) // want `len\(\[\]int32\(s\)\) decodes and allocates the whole rune slice just to count it; utf8\.RuneCountInString\(s\) is the bit-identical count with zero allocation`
}

// []byte form: only the conversion is dropped, the outer len stays.
func byteCount(s string) int {
	return len([]byte(s)) // want `len\(\[\]byte\(s\)\) copies the whole string just to take its length; len\(s\) is the identical byte count with zero allocation`
}

// []byte inside a larger expression: still just len(s), the surrounding
// len call already delimits it.
func byteExpr(s string) int {
	return 2 * len([]byte(s)) // want `len\(\[\]byte\(s\)\) copies the whole string just to take its length; len\(s\) is the identical byte count with zero allocation`
}

// []uint8 spells the identical []byte type.
func byteUint8(s string) int {
	return len([]uint8(s)) // want `len\(\[\]uint8\(s\)\) copies the whole string just to take its length; len\(s\) is the identical byte count with zero allocation`
}
...
func commentedRune(s string) int {
	return len( // keep me // want `len\(\[\]rune\(s\)\) decodes and allocates the whole rune slice just to count it; utf8\.RuneCountInString\(s\) is the bit-identical count with zero allocation`
		[]rune(s))
}

func commentedByte(s string) int {
	return len([]byte( // keep me // want `len\(\[\]byte\(s\)\) copies the whole string just to take its length; len\(s\) is the identical byte count with zero allocation`
		s))
}
...
func shadowedUtf8(s string) int {
	utf8 := 0
	_ = utf8
	return len([]rune(s)) // want `len\(\[\]rune\(s\)\) decodes and allocates the whole rune slice just to count it; utf8\.RuneCountInString\(s\) is the bit-identical count with zero allocation`
}
